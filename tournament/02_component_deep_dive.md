...
2026-04-14 delta recheck: `origin/tsyhan`, `origin/kurdupel`, and `origin/volynets` were refreshed and re-evaluated from non-Markdown files only. The old `origin/monero-privacy-system` branch is now `origin/tsyhan`. API topic itself is not a scoring factor.

`volynets` was initially excluded (only a `LICENSE` file), but later pushes turned it into a full VM/Ansible implementation. The newest recheck improves its VM/systemd score, but it still does not change the best-in-class verdicts for Docker, Terraform, RabbitMQ, or Redis.
...
| tsyhan | React 18 + Recharts | Dev-build via npm on VM / dev container | `REACT_APP_API_URL` (build-time) | No production frontend Dockerfile |
...
| smoliakov | Go | ctx-aware | 15s | LimitReader bounds upstream response, atomic counters | Basic |
| kurdupel | Python/Flask | — | 5s | Background thread for periodic refresh, price-smoothing | Minimal |
| kazachuk | Go | Cache fallback for CoinGecko (60s TTL) | — (**no HTTP timeout**) | Goroutine pool; missing timeouts are a hang risk | Minimal |
| tsyhan | Nginx (static proxy) + FastAPI | — | — | Reverse proxy only; logic lives in backend | FastAPI validation |
| zakipnyi | FastAPI | — | — | CORS `*`, no retries at proxy layer | Minimal |
| penina | Flask | Stale-cache fallback | 5s | Redis-backed TTL cache | Minimal |
| shturyn | Go | — | 3s client | Cache-aside pattern | Minimal |
...
| kazachuk | **auto-ack** (loses messages on crash) | No — duplicates possible | DB failure not protected by ACK semantics | 5s sleep loop | Flask, LIMIT 10000 hard-coded |
| shturyn | auto-ack | UPSERT on `time` column | DB failure not protected by ACK semantics | 5s backoff | FastAPI `/history` (last 50), `/today` |
| smoliakov | Manual ACK, signal-handling, UPSERT | UNIQUE(cc, exchange_date, collected_at) | UPSERT path, basic error handling | Auto-reconnect 5s | Django read-through cache |
| kurdupel | Manual ACK after `saveToDB`, but DB insert errors are swallowed | 10-second duplicate window (heuristic) | Insert errors can still be ACKed | 5s backoff | Go service with time-bucketing for charts |
| penina | Auto-ack | No constraint | None | None | Go consumer, infinite loop |
| zakipnyi | Auto-ack | No constraint | DB failure not protected by ACK semantics | 5s backoff | FastAPI read endpoints |
| tsyhan | RabbitMQ notification after DB write, no queue consumer | Primary keys / indexes; no event replay ID | No ACK path for persistence because worker writes directly to Postgres | Not applicable for persistence | FastAPI read API; queue is not the persistence boundary |
...
| tsyhan | 5 tables, well-indexed | Primary-key/id based tables; no event replay key | **async pool_size=10, overflow=20** (SQLAlchemy) | Env creds | Static `schema.sql`; Alembic NOT wired up despite SQLAlchemy |
...
| shturyn | — | — | Not present |
| tsyhan | FastAPI session storage through Redis | Required for sessions; no fallback pattern seen | Good external-state direction, but needs managed Redis/secrets story |
...
| tsyhan | Default exchange notification route `monero_events` | Not configured as durable persistence path | Worker publishes after DB commit | No consumer ACK path | `aio_pika.connect_robust` in worker | — |
...
|---|---|---|---|
| **Shabat** | **Terraform (taliesins/hyperv) + cloud-init** | High — static MACs, dynamic memory, WSL/Hyper-V-specific implementation | Gen 2 Secure Boot, local-hypervisor specific |
| **tsyhan** | **Terraform (libvirt) + cloud-init** | High — Alpine 3.19 + OpenRC, separate app/db/redis/rabbitmq VMs, network DNS | Cron-based auto-deploy every 60s; default branch variable still stale |
| kurdupel | Vagrant (4 Ubuntu VMs) + Ansible roles | Medium-High — private net 192.168.56.x/24, but inventory has local private-key paths | Clean role-per-service split; runtime tied to `/vagrant` |
...
**Best-in-class: Shabat.** Terraform + cloud-init, documented operational quirks, and a full Ansible deploy layer on top is the closest any branch gets to "lift-and-shift-able to a real cloud provider". **tsyhan** is a close second — arguably its Terraform structure is slightly cleaner (`main.tf` / `variables.tf` / `outputs.tf` separation, cloud-init templating) — but Shabat's Ansible deployment pipeline closes the gap.

**Cherry-pick from Shabat:** `terraform/vms.tf`, `terraform/provider.tf`, and the working Terraform/cloud-init wiring.
**Cherry-pick from tsyhan:** `terraform/main.tf` cloud-init template pattern, plus its separate Redis/RabbitMQ VM modeling.
...
| penina | Yes | python:3.10-slim, debian:bookworm-slim | No | On RabbitMQ | Docker attempt exists, but runnable drift remains |
| tsyhan | Yes (backend only) | python:3.11-slim | **Yes** (UID 1001) | In Dockerfile + compose | Full dev compose now includes Redis/RabbitMQ; missing frontend prod Dockerfile |
| kurdupel | — | — | — | — | No Docker |
| smoliakov | — | — | — | — | No Docker |
| zakipnyi | — | — | — | — | No Docker |
...
|---|---|---|---|---|
| **tsyhan** | libvirt | **main.tf / variables.tf / outputs.tf** + cloud-init templates | Medium (libvirt-locked) | Local state, no backend |
...
**Best-in-class: tsyhan** for structure and templating patterns; **Shabat** for operational completeness; **smoliakov** is the only branch with an AWS provider at all.

The honest read: nobody has cloud-ready Terraform. Shabat and tsyhan both have good *patterns* but are locked to their local hypervisor. smoliakov has an AWS provider but the module is too thin to be useful on its own.

**Recommended approach for the merged baseline:** take the **structure** from tsyhan (`main.tf` / `variables.tf` / `outputs.tf` split, cloud-init templating), the **env-var discipline** from Shabat, and then re-target the provider to AWS using smoliakov's starting point as a reference.
...
| tsyhan | `.env` templates, `.env.template` files, non-root backend container | Partial — Terraform state still exposes plaintext passwords; demo defaults remain |
...
| kurdupel | Ansible Vault file exists, but inventory still hardcodes a dev macOS private-key path and Redis protected mode is disabled | Partial |
| penina | **Credentials in app.py, main.go, definitions.json** | No |
| zakipnyi | **`coinops123` in Vagrantfile, app.py, consumer.py** | No |
...
|---|---|---|---|
| **tsyhan** | **structlog** (JSON-ready) | `/health` on API + Docker healthchecks | None |
...
**Best-in-class: tsyhan.** It's the only branch that chose a structured-logging library from day one. Everything else will need a logging refactor before ELK/Loki/CloudWatch ingestion is useful. The effort to retrofit `structlog` onto Shabat/hrenchevskyi is small — but it still has to happen.

**Cherry-pick from tsyhan:** the `structlog` bootstrap and the Pydantic `Settings` pattern (`backend/config.py` with `lru_cache`).
