...
variable "ansible_user" {
  description = "SSH user written into the generated Ansible inventory"
  type        = string
  default     = "vagrant"
}
...
variable "run_ansible" {
  description = "Run Ansible provision and deploy from Terraform after VMs are created"
  type        = bool
  default     = false
}

variable "ansible_env_file" {
  description = "Shell env file sourced before Terraform runs Ansible"
  type        = string
  default     = "../.env"
}

variable "ansible_ssh_wait_timeout_seconds" {
  description = "How long Terraform waits for SSH before running Ansible"
  type        = number
  default     = 300
}
