locals {
  ...
  cloudflare_token_secret_ref   = try(var.secret_refs.cloudflare_token.secret_id, try(var.secret_refs.cloudflare_token.name, ""))
  ...
  cloud_children = compact([
    "bastion",
    "app",
    local.database_managed ? "" : "db",
    length(var.k3s_names) > 0 ? "k3s" : "",
  ])
  ...
  [backend]
  ${join("\n", local.app_inventory_lines)}${local.db_inventory_block}${local.k3s_inventory_block}
