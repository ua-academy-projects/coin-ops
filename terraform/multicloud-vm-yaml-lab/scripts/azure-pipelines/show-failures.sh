...
mapfile -t failed_logs < <(python3 -c 'import json,sys; rows=json.load(sys.stdin).get("records", []); [print("{}\t{}\t{}".format(r.get("type",""),r.get("name",""),r.get("log",{}).get("id",""))) for r in rows if r.get("result") == "failed" and r.get("log",{}).get("id")]' <<<"$timeline")
