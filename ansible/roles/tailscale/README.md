...
**Where it runs:** every cloud host (via [[cloud_app_stack]] and
[[cloud_db_stack]]). Skipped entirely when `tailscale_auth_key` is empty,
so deploys still succeed before the key is provisioned.

**Reads:** `tailscale_auth_key` (usually published by [[cloud_secrets]]),
