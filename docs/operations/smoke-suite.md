...
```bash
./deployments/deployments/smoke/smoke.sh postgres-runtime
```
...
```bash
./deployments/deployments/smoke/smoke.sh             # up → wait → check → down (default)
./deployments/deployments/smoke/smoke.sh --keep      # same, but leave the stack running on pass
./deployments/deployments/smoke/smoke.sh up          # just start the stack
./deployments/deployments/smoke/smoke.sh check       # run checks against an already-running stack
./deployments/deployments/smoke/smoke.sh logs        # tail logs (add a service name for just one)
./deployments/deployments/smoke/smoke.sh down        # stop and remove the stack

./deployments/deployments/smoke/smoke.sh postgres-runtime          # postgres mode: up → check → down
./deployments/deployments/smoke/smoke.sh postgres-runtime --keep   # leave postgres-mode stack running
./deployments/deployments/smoke/smoke.sh postgres-runtime up
./deployments/deployments/smoke/smoke.sh postgres-runtime check
./deployments/deployments/smoke/smoke.sh postgres-runtime down
```
...
```bash
./deployments/deployments/smoke/smoke.sh logs history-api     # per-service logs
./deployments/deployments/smoke/smoke.sh logs                 # all logs
./deployments/deployments/smoke/smoke.sh down                 # tear down when done debugging
```
...
compose network. Override with `SMOKE_GATEWAY_URL=... ./deployments/deployments/smoke/smoke.sh` if the
...
Override with `SMOKE_WAIT_TIMEOUT=300 ./deployments/deployments/smoke/smoke.sh` on slower machines or
...
1. Write a bash function in `deployments/smoke/smoke.sh` that exits 0 on pass, non-zero
