...
The current repository supports a multi-cloud deployment flow built around:

- Terraform infrastructure in `terraform.gcp.aws/`
- Ansible provisioning and deployment
- Docker Compose runtime services on VMs

The deployed shape depends on the cloud:

| Cloud | Main nodes | Database mode | Public entrypoint |
| --- | --- | --- | --- |
| AWS | bastion, app, web, load balancer | AWS RDS PostgreSQL | ELB DNS name or Cloudflare |
| GCP | bastion, app, web, load balancer | local PostgreSQL on app VM | load balancer IP or Cloudflare |

### Current runtime responsibilities

| Node | Role | Main services |
| --- | --- | --- |
| bastion | admin access | SSH jump host only |
| app | backend runtime | history-api, history-consumer, proxy, RabbitMQ, Redis, and PostgreSQL in GCP/local-db mode |
| web | browser-facing gateway | nginx + React UI |

### Cloud-specific database behavior

- **AWS path** can use **external PostgreSQL through RDS**
- **GCP path** currently follows the older **local database flow on the app VM**
- in the AWS RDS flow, the app uses `RUNTIME_BACKEND=external`
...
The "three services across separate VMs" shape is the smallest useful distributed system. It teaches the concepts (network boundaries, message queues, shared state, externalized storage) without being overwhelming.
...
The deployment flow now creates infrastructure in cloud VPC/VNet style networks rather than the original fixed Hyper-V lab only.

Common shape:

- one **bastion** node with public SSH access
- one **app** node on a private subnet
- one **web** node on a private subnet
- one **public load balancer**
- optional **external managed database** in AWS

### Why inventory generation matters

Terraform writes `ansible/inventory.generated` automatically from outputs.

Why this is important:

- Ansible always targets the current bastion, app, and web IPs
- cloud recreation does not require manual inventory editing
- SSH proxying through bastion is generated automatically

This is a major improvement over the original static-IP-only workflow.
...
Load Balancer
  |
  v
web VM (nginx + React UI)
  |
  +-- /api ---------> app VM :8080 (proxy)
  |
  +-- /history-api -> app VM :8000 (history-api)
...
app VM
  ...
  - RabbitMQ
  - Redis
  - PostgreSQL (GCP/local-db path only)
  - external RDS connection (AWS path only)

bastion VM
  - SSH jump host for private nodes
```

### Networking differences by cloud

| Cloud | Public LB exposure | Cloudflare record type | Database mode |
| --- | --- | --- | --- |
| AWS | ELB DNS name | `CNAME` | RDS or separate/local VM flow |
| GCP | load balancer IP | `A` record | local PostgreSQL on app VM |

### Service ports

Regardless of cloud, the internal runtime still uses:

- `8080` for proxy
- `8000` for history API
- `5672` for RabbitMQ in external runtime mode
- `5432` for PostgreSQL

The browser should not call private node IPs directly. It should use the public gateway only.

### Why Redis binds to localhost only

Redis is a runtime helper, not a public service. It should remain reachable only from the local app runtime where possible.
...
- **Proxy (`/api`)** serves *live data* — current markets, whale positions, prices, session state. It's the thing the UI hits on every refresh tick.
- **History API (`/history-api`)** serves *stored data* — historical snapshots read from PostgreSQL. The UI hits it on demand (user clicks a chart).
...
Older sections of this document still reference the original `172.31.x.x`
three-node lab layout. Treat those examples as historical architecture notes,
not as the current required addressing model for AWS/GCP deployments.
...
`ansible/inventory.generated` maps hostnames to IPs and groups hosts into logical sets.

Today it is generated automatically by Terraform and typically looks like:

```ini
[history]
coinops-app ansible_host=<private-app-ip>

[proxy]
coinops-app ansible_host=<private-app-ip>

[ui]
coinops-web ansible_host=<private-web-ip>
```

When you run `ansible-playbook -i ansible/inventory.generated deploy.yml`, Ansible reads this file to know who to connect to and what group each host belongs to.
...
ansible-playbook -i ansible/inventory.generated ansible/provision.yml --limit softserve-node-02
ansible-playbook -i ansible/inventory.generated ansible/deploy.yml --limit softserve-node-02
