...
- VMs provisioned with static IPs (see [blockers.md](../operations/blockers.md) for the Hyper-V one-time fix)
...
SSH rejects key files that are readable by other users. Vagrant regenerates keys with open permissions on every `vagrant up`, so this must be re-run after each `vagrant up`. See [blockers.md](../operations/blockers.md) blocker #12 for details.
