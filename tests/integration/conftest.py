...
BOOTSTRAP_SCRIPT_PATH = REPO_ROOT / "tests" / "integration" / "postgres_bootstrap.sh"
...
def database_url():
    postgres = (
        ...
        .with_command("postgres -c shared_preload_libraries=pg_cron -c cron.database_name=coinops")
    )
    with postgres:
...
def history_api_module(database_url):
    return load_module("services/history/main.py", {"DATABASE_URL": database_url})


@pytest.fixture(scope="session")
def runtime_consumer_module(database_url):
    return load_module("services/runtime/src/runtime_consumer.py", {"DATABASE_URL": database_url})
