...
terraform -chdir=terraform.gcp.aws apply
...
ansible-playbook -i ansible/inventory.generated ansible/deploy.yml --limit softserve-node-02,localhost
...
- Base VHDX must be pre-resized to ≥20 GB with `Resize-VHD` in PowerShell Admin before `terraform -chdir=terraform.gcp.aws apply`. Cloud-init `growpart` expands the partition automatically on first boot.
...
- Stop VMs before `terraform -chdir=terraform.gcp.aws apply` when changing hardware (DVD drive resource pool lock).
